...
def new_race(env, actor, races=100):
    print("NEW RACES ")
    total_rewards = []
    total_steps = []
...
steps,rewards = new_race(env=racer, actor= actor, races=100)
avg_tested_reward = tf.reduce_mean(rewards)
print(f"test DDPG : mean trajectories steps :  {tf.reduce_mean(steps)}. mean rewards : {avg_tested_reward}")
